...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $publisher=$env:A90_PUBLISHER; $password=[string]$env:A90_PASSWORD; $exe=$env:A90_EXE; $pfx=$env:A90_PFX; $tool=$env:A90_SIGNTOOL; $cert=Get-ChildItem Cert:\\CurrentUser\\My | Where-Object { $_.Subject -eq ('CN='+$publisher) -and $_.HasPrivateKey } | Select-Object -First 1; if (-not $cert) { $cert=New-SelfSignedCertificate -Type CodeSigningCert -Subject ('CN='+$publisher) -FriendlyName $publisher -CertStoreLocation Cert:\\CurrentUser\\My -HashAlgorithm SHA256 -NotAfter (Get-Date).AddYears(2) }; $secure=ConvertTo-SecureString $password -AsPlainText -Force; Export-PfxCertificate -Cert $cert -FilePath $pfx -Password $secure | Out-Null; if (-not (Get-ChildItem Cert:\\CurrentUser\\TrustedPublisher | Where-Object Thumbprint -eq $cert.Thumbprint)) { $cer=Join-Path $env:TEMP 'a90-private-test.cer'; Export-Certificate -Cert $cert -FilePath $cer | Out-Null; Import-Certificate -FilePath $cer -CertStoreLocation Cert:\\CurrentUser\\TrustedPublisher | Out-Null }; & $tool sign /fd SHA256 /f $pfx /p $password $exe; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; & $tool verify /all /v $exe; if ($LASTEXITCODE -ne 0) { Write-Warning 'The self-signed certificate is not publicly trusted; this is expected for private testing.' }; exit 0"
set "RESULT=%ERRORLEVEL%"
if not "%RESULT%"=="0" exit /b %RESULT%
echo Signed for this Windows user: "%EXE%"
echo Do not distribute "%PFX%".
endlocal
